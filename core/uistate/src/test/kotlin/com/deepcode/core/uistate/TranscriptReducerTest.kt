...
class TranscriptReducerTest {
    ...
    fun `工具状态随事件正确流转`() {
        ...
        reducer.apply(
            ToolCallApproved(newEventId(), sessionId, turnId, 0L, echoCall, com.deepcode.core.model.ApprovalScope.ONCE)
        )
    ...
    fun `回放历史事件与增量处理结果完全一致`() {
        val events: List<AgentEvent> = listOf(
            ...
            TurnCompleted(
                ...
                com.deepcode.core.model.StopReason.END_TURN, Usage(10, 5), 1,
            ),
        )
