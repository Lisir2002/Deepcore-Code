...
# ---------------------------------------------------------------------------
# 七·六、core:logging（日志门面）—— 决策 D2/D7/D25，Release 包也要能落盘可读
# ---------------------------------------------------------------------------
# 全保留：
#   · LogEntry 是 JSON 行序列化的载体，字段名被混淆后日志文件彻底没法读；
#   · 崩溃栈里要能看见 Log / LogSink / CrashVault 的调用点，便于定位。
# LogcatSink 只在 debug 构建 plant（BuildConfig.DEBUG），release 分支被 R8
# 裁掉后该类即死代码，会被自动移除——这里刻意不 keep，避免膨胀。
-keep class com.deepcode.core.logging.** { *; }
# app 层日志实现（文件双写 / 崩溃捕获 / 导出）同样保留，保证 Release 崩溃栈可读。
-keep class com.deepcode.agent.logging.** { *; }
